      ...
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-[#ad7f4acb] dark:bg-[#f1b773cb] block rounded-3xl"
          ...
          <Card>
            {item.ico && <span className="block dark:bg-red-500/25 bg-red-600 w-max p-2 rounded-xl">{item.ico}</span>}
